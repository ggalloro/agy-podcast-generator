...
* **Article Retrieval:** Upon triggering, the application will fetch the latest articles from all active RSS feeds.
* **Article Summarization:** Each fetched article will be automatically summarized using Gemini to extract key information and present it concisely.
...
* **RSS Feed URLs:** The application will store the RSS feed URLs provided by the user.
...
* **Article Summaries:** Summaries may be stored temporarily during the podcast generation process, or optionally associated with a generated podcast for future reference.